...
@interface IDMPhotoBrowser () {
    ...
    UIPageControl *_counterPageControl;
...
- (void)setPhotoURLs:(NSArray *)photoURLsArray {
...
- (void)viewDidLoad {
    ...
    //_counterPageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, 0, 95, 40)];;
...
- (void)updateToolbar {
	...
	if ([self numberOfPhotos] > 1) {
        //_counterPageControl.numberOfPages = [self numberOfPhotos];
        //_counterPageControl.currentPage = _currentPageIndex+1;
        
        NSUInteger totalNumber = [self numberOfPhotos];
        if (self.totalNumber > 0) {
            totalNumber = self.totalNumber;
        }
		_counterLabel.text = [NSString stringWithFormat:@"%u %@ %u", _currentPageIndex+1, IDMPhotoBrowserLocalizedStrings(@"/"), totalNumber];
	} else {
