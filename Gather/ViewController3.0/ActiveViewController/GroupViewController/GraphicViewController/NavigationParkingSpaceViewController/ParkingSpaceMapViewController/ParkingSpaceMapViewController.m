//
//  ParkingSpaceMapViewController.m
//  Gather
//
...
@interface ParkingSpaceMapViewController ()<BMKMapViewDelegate, BMKLocationServiceDelegate>  {
...
@implementation ParkingSpaceMapViewController
...
- (void)addPointAnnotation
{
    ...
    pointAnnotation.coordinate = CLLocationCoordinate2DMake(self.addressEntity.lat, self.addressEntity.lon);
    pointAnnotation.title = self.addressEntity.addr_name;
    
    _mapView.centerCoordinate = CLLocationCoordinate2DMake(self.addressEntity.lat, self.addressEntity.lon);
    [_mapView addAnnotation:pointAnnotation];
    
    for (int i=0; i<self.addressEntity.parking_spaces.count; i++) {
        ParkingSpaceEntity *entity = [self.addressEntity.parking_spaces objectAtIndex:i];
        
        CustomPointAnnotation* pointAnnotation = [[CustomPointAnnotation alloc]init];
        pointAnnotation.tag = i+1;
...
- (BMKAnnotationView *)mapView:(BMKMapView *)view viewForAnnotation:(id <BMKAnnotation>)annotation
{
    ...
    if (((CustomPointAnnotation*)annotation).tag == 0) {
        ((BMKPinAnnotationView*)annotationView).image = image_with_name(@"img_active_end_pin");
    }else {
        ((BMKPinAnnotationView*)annotationView).image = image_with_name(@"img_parking_space_pin");
    }
...
- (void)mapView:(BMKMapView *)mapView annotationViewForBubble:(BMKAnnotationView *)view {
    ...
    if (annotation.tag == 0) {
        self.endCoor = CLLocationCoordinate2DMake(self.addressEntity.lat, self.addressEntity.lon);
        self.endName = self.addressEntity.addr_name;
    }else {
        ParkingSpaceEntity *entity = [self.addressEntity.parking_spaces objectAtIndex:(annotation.tag - 1)];
